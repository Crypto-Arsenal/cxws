...
class BinanceFuturesUsdtmPrivateClient extends BinancePrivateBase_1.BinancePrivateBase {
    constructor({ useAggTrades = true, requestSnapshot = true, socketBatchSize = 200, socketThrottleMs = 1000, restThrottleMs = 1000, testNet = false, 
    ...
    wssPath = "wss://fstream-auth.binance.com/streams", restL2SnapshotPath = "https://fapi.binance.com/fapi/v1/depth", watcherMs, l2updateSpeed, l2snapshotSpeed, batchTickers, apiKey = "", apiSecret = "", } = {}) {
        if (testNet) {
                    ...
                    try {
                        that.ccxt
                            ...
                            if (d.listenKey != that.apiToken) {
                                console.log("fapiPrivatePostListenKey listenKey renewal expired key- reconnecting", d.listenKey, that.apiToken, new Date());
                                clearTimeout(that._listenKeyAliveNesstimeout);
                                that.reconnect();
                                return;
                            }
                            console.log("fapiPrivatePostListenKey listenKey extended", d.listenKey, that.apiToken, new Date());
